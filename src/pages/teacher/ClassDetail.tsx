...
export default function ClassDetail() {
    ...
    const { classData, error: classError, isLoading : classLoading } = useClass(Number(classId))
    const { students, error: studentsError, isLoading: studentsLoading } =useStudentsByClass(numericClassId);
    const { assignments, error: assignmentsError, isLoading : assignmentsLoading} = useAssignmentsByClass(Number(classId));
    ...
    if (classError || studentsError || assignmentsError || !classData) {
      return <ErrorMessage message="Class details could not be loaded." />;
    }
