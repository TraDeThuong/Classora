...
export default function StudentCompleteSignUp() {
  ...
  async function onSubmit(data: CompleteSignupFormData) {
    try {
      ...
      const student = await completeStudentGoogleSignup(data.password);
      ...
      const message =
        error instanceof Error
          ? error.message
          : "Could not complete student signup";

      if (message === STUDENT_TEACHER_ACCOUNT_ERROR) {
      ...
      <div className="w-full max-w-xl rounded-3xl border border-white/20 bg-white/10 p-6 shadow-xl backdrop-blur-xl sm:p-8">
        <div className="mb-8 text-center">
          ...
          <p className="mt-2 text-white/60">
            Create a password for your Classora student account
          </p>
        </div>
