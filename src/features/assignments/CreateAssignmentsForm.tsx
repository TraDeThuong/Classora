...
import type { Assignment, AssignmentQuestion } from "../../types/assignments";
...
export default function CreateAssignmentsForm({onCloseModal, assignmentToEdit} : CreateAssignmentsFormProps) {
  ...
  const editId = assignmentToEdit?.id ?? null;

  const formatQuestionsForForm = (
  ...
  const {
    ...
    defaultValues: assignmentToEdit
      ? {
  ...
  <div className = "flex w-full flex-col items-center px-4 sm:px-6">
    <h1 className = "mb-5 text-center font-bold text-brand-100" >CREATE ASSIGNMENT </h1>
    <form onSubmit={handleSubmit(onSubmit)} className="flex max-h-[80vh] w-full max-w-5xl flex-col gap-6 overflow-y-auto px-1 pb-4">
      ...
      <div className="flex flex-col gap-5">
        {fields.map((field, index) => {
          ...
          return (
            <div
              key={field.id}
              className="w-full rounded-3xl border border-white/20 bg-white/10 p-4 backdrop-blur-xl sm:p-6"
            >
      ...
      <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:justify-end">
        <button
          type="submit"
          disabled={isWorking}
          className="w-full rounded-2xl bg-brand-200 px-6 py-3 text-base font-semibold text-white transition-all hover:bg-brand-300 disabled:cursor-not-allowed disabled:opacity-60 sm:w-auto sm:text-xl"
        >
          {isWorking
            ? isEditSession
              ? "Updating..."
