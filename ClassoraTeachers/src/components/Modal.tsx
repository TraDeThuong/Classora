...
function Window({ children, name }: WindowProps) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-1000 flex items-center justify-center bg-black/30 p-4 backdrop-blur-sm transition-all duration-500">
      <div
        ref={ref}
        className="relative w-full max-w-3xl overflow-hidden rounded-3xl border border-white/20 bg-white/10 p-8 shadow-xl backdrop-blur-xl transition-all duration-500"
      >
        <button
          onClick={close}
